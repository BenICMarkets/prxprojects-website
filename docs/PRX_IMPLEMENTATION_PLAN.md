...
1. **Relationship to Plexi Renovations (answered by Ben).** Same owners; PRX Projects is the larger relaunch of Plexi Renovations, or possibly a separate division of it. That allows using the Plexi track record, projects and reviews, but the wording must be accurate. Working wording until confirmed: "PRX Projects, from the team behind Plexi Renovations", not "formerly" or "a division of" (those need a definite answer). Still open: which legal entity holds NHBRC/CIDB/COIDA/insurance, and whether "13+ years" may be claimed under the PRX name.
